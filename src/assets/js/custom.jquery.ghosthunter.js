...
var pluginMethods	= {
    ...
    init			: function( target , opts ){
        ...
        this.index = lunr(function () {
            ...
            //this.add({title:1,description:'hey', link:'hey', markdown:'hey', pubDate:'hey' ,tag:'hey', id:'1'});
        });
        this.index_tags = lunr(function () {
            this.field('name', {boost: 10})
            this.field('slug')
            this.ref('id')
        });
        this.index_users = lunr(function () {
            this.field('name', {boost: 10})
            this.field('slug')
            this.ref('id')
        });
        ...
        if( opts.onKeyUp ) {
            target.keyup(function() {
                ...
                that.find(target.val());
                that.find_tags(target);
                //that.find_users(target);
                }
            });
    ...
    /*	Here we load all of the blog posts to the index. 
        ...
        var index 		= this.index,
            ...
            obj			= {limit: "all",  include: "tags", order: "published_at DESC"};
							if  ( this.includepages ){
								obj.filter="(page:true,page:false)";
							}

        api.posts.browse(obj).then((data) =>
        {
            ...
            data.forEach(function(arrayItem){
                ...
                blogData[arrayItem.id] = {
					...
					pubDate: tempdate,
					image: arrayItem.feature_image == null ? domain + "/assets/images/ippon-bkr.png" : String(arrayItem.feature_image ), 
					link: arrayItem.url,tag: category
				};
            });
        });
    ...
    find 		 	: function(value){
        ...
        {
            if(this.displaySearchInfo) $('.info_search').empty().append(this.format(this.info_template,{"amount":searchResult.length}));
        }
        ...
        sessionStorage.setItem("search",JSON.stringify(resultsData));

        if(this.before) {
            this.before(resultsData);
        };
        /*var result_slice = resultsData.slice(0,6);*/
        if(this.onComplete) {
            this.onComplete(resultsData);
        };
    },
    ...
    find_tags: function(target){
        this.isInit = true;
        $('.search_tag ul').empty();
        var index 		= this.index_tags,
            blogDataTag = [];
            api.tags.browse({limit:'all', include: 'tags,authors,count.posts', order:'count.posts DESC',}).then((data) =>
            {
            //searchData = data.tags;
            data.forEach(function(arrayItem){
                var parsedData 	= {
                    id 			: String(arrayItem.id),
                    name 		: String(arrayItem.name),
                    slug	: String(arrayItem.slug),
                    created_at 	: String(arrayItem.created_at)

                }


                index.add(parsedData);
                blogDataTag[arrayItem.id] ={name: arrayItem.name,slug: arrayItem.slug};
            });
            
            var searchResult = index.search(target.val()),
                searchResult_slice = searchResult.slice(0,4);
                if(searchResult.length==0){
                    $('.search_tag').css('display','none');
                }else{
                    $('.search_tag').css('display','block');
                }
                $('.search_tag ul').empty().append(
                    $.map(searchResult_slice,function(value,index){
                        var lunrref		= value.ref;
                        var postData  	= blogDataTag[lunrref];
                        return "<li><a href='../../tag/"+postData.slug+"'>"+postData.name+"</a></li>";
                    }).join(" ")
                );
        ...
        results: "#popover-content .search_post",
        zeroResultsInfo     : false,
        before: function(result){
            $('.search_tag ul').empty();
            if(result.length==0){
                $('.post_zone').css('display','none');
            }else{
                $('.post_zone').css('display','block');
            }
        },
        onKeyUp: true,
        zeroResultsInfo		: true,
        info_template   : "Number of posts found: {{amount}}",
        result_template : '<li><img src="{{image}}" alt=""><a href="{{link}}">{{title}}</a></li>',
